...
TYPED_TEST(TestWarping, FindCorrespondenceJacobianPose)
{
  ...
  typedef typename TestFixture::SE3T SE3T;
  typedef typename Eigen::Matrix<TypeParam,1,2> GradT;  
  ...
  //LOG(INFO) << "Bilinear value"<<
  Eigen::Matrix<TypeParam,2,6> jacobian = df::FindCorrespondenceJacobianPose(corresp, dpt, cam, pose);
  //vc::Image2DView<GradT, vc::TargetHost> grad1;
 
  //LOG(INFO) << "Bilinear value"<< grad1.template getBilinear<GradT>(corresp.pix1); 
...
//   SE3T pose1 = SE3T::exp(twist1);
  ...
  Vec3 tpt = pose10 * pt;
  LOG(INFO) << "Relative pose10 = " << pose10;
  LOG(INFO) << "tpt = " << tpt;
