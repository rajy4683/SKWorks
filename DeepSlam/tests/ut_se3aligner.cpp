...
public:
  ...
  typedef typename AlignerT::ReductionItem ReductionItemT;
  ...
  */
  ReductionItemT hostLK(const Sophus::SE3<Scalar> se3)
                                      //const typename ImageBuffer img0,
                                      //const typename ImageBuffer img1,
                                      //const typename ImageBuffer dpt0,
                                      //const typename GradientBuffer grad1)
                                      //vc::Buffer1DView<typename BaseT::ReductionItem, vc::TargetHost> bscratch)
  {
    //typedef typename SE3Aligner<Scalar>::ReductionItem Item;
    typedef typename AlignerT::ReductionItem Item;
    int w = img0_.cols;
    int h = img0_.rows;

    GradientBuffer<vc::TargetHost> localgradcpu(w, h);
    ImageBuffer<vc::TargetHost> img0(w,h);
    ImageBuffer<vc::TargetHost> img1(w,h);
    ImageBuffer<vc::TargetHost> dpt0(w,h);
    localgradcpu.copyFrom(*grad1gpu_);
    img0.copyFrom(img0_);
    dpt0.copyFrom(dpt0_);
    img1.copyFrom(img1_);
    Item sum;
    //LOG(INFO)<< "Grad values:"<<localgradcpu(0,0) << " " << localgradcpu(0,1) 
    //         << " " << localgradcpu(1,0) << " " << localgradcpu(1,1);
    LOG(INFO)<< "Grad dim:"<<localgradcpu.width() << " " << localgradcpu.height(); 

    //vc::runReductions(img0.area(), [&] __device__ (unsigned int i)
    for (int i=0;i<img0.area();++i)
    {
      const unsigned int y = i / w;
      const unsigned int x = i - (y * w);
      
      sum += df::LucasKanadeSE3<Scalar, 
                               vc::TargetHost, 
                               Sophus::SE3<Scalar>, 
                               df::PinholeCamera<Scalar>,
                               Item,
                               Eigen::Matrix<Scalar,1,2>>(x, y, se3, cam_, img0, img1, dpt0, localgradcpu, 0.1f);
    }
    LOG(INFO) << "LK sum: " << sum.residual << " LK Residual: "<<sum.inliers; 
    return sum;
  ...
  //LucasKanadeSE3(x, y, se3, cam, img0, img1, dpt0, grad1, huber_delta);
...
TYPED_TEST(SE3AlignerTest, FullJacobianTest)
{
  ...
  for (int i = 0; i < this->eps_steps_; i++)
  {
   ...
   /* 
   LOG(INFO) << "Init Pose:" <<std::endl 
           <<"Rot:"<<std::endl<<pose_0.so3().matrix()<<std::endl
           <<"Translation:"<<std::endl<<pose_0.translation()<<std::endl
           <<"Residual:"<<result_0.residual <<std::endl
           <<"Residual.JTJ:"<<result_0.JtJ<<std::endl
           <<"Residual.JTR:"<<result_0.Jtr <<std::endl;
   */
    ...
    for (int i = 0; i < 6; ++i)
    {
      ...
      auto result = this->hostLK(pose);
      LOG(INFO) << "Solving Hessian:"<< -result.JtJ.toDenseMatrix().ldlt().solve(result.Jtr); 
      /*LOG(INFO) << "Perturbed Pose:" <<std::endl 
           <<"Rot:"<<std::endl<<pose.so3().matrix()<<std::endl
           <<"Translation:"<<std::endl<<pose.translation()<<std::endl
           <<"Residual:"<<result.residual <<std::endl
           <<"Residual.JTJ:"<<result.JtJ<<std::endl
           <<"Residual:"<<result.Jtr <<std::endl;*/
