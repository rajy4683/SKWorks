...
TEST_F(TestSfmAligner, CorrespondenceJacobianCode)
{
  ...
  typedef typename vc::Image2DManaged<float, vc::TargetHost> ImageBufferFT;
  ...
  int cs = code.rows();
  // render new depth
  ImageBufferFT dpt(width, height);
  ImageBufferFT::ViewT code_jac = pyr_jac[0];
  ImageBufferFT::ViewT prx_0code = pyr_prx[0];
  for (int x = 0; x < width; ++x)
  {
    for (int y = 0; y < height; ++y)
    {
      Eigen::Map<const Eigen::Matrix<float,-1,-1>> prx_J_cde(&code_jac(x*cs, y), 1, cs);
      dpt(x,y) = df::DepthFromCode(code, prx_J_cde, prx_0code(x,y), avg_dpt);
      LOG(INFO)<<"Jacobian of code * code:" << prx_0code(x,y)+(prx_J_cde*code)(0) << " dpt("<<x<<","<<y<<")="<<dpt(x,y);
    }
  }
