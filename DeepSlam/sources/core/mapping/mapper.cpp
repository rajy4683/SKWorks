...
//  int pyrlevels = map->keyframes.Last()->pyr_dpt.Levels();
  ...
  for (int lvl = 0; lvl < pyrlevels; ++lvl)
  {
    ...
    mosaic = CreateMosaic(array, pairs.size(), 5);
                                      ...
                                      double ts1, double ts2)
{
  ...
  pipeline_controller_.AddFactorTask<work::InitPriorsTask<Scalar,CS>>(kf0, opts_.code_prior, opts_.pose_prior);
  pipeline_controller_.AddFactorTask<work::InitPriorsTask<Scalar,CS>>(kf1, opts_.code_prior);

  pipeline_controller_.AddFactorTask<work::PhotometricTask<Scalar,CS>>(kf0, kf1, opts_.pho_iters,
                                                        cam_pyr_, aligner_);
  pipeline_controller_.AddFactorTask<work::PhotometricTask<Scalar,CS>>(kf1, kf0, opts_.pho_iters,
                                                        cam_pyr_, aligner_);

  // optimize until there is no more work
  // this means either convergence or running out of iters
  while (!pipeline_controller_.Empty())
    MappingStep();
}
                                     ...
                                     const Features& ft)
{
  ...
  map_->AddKeyframe(kf);
  pipeline_controller_.AddFactorTask<work::InitPriorsTask<Scalar,CS>>(kf, opts_.code_prior,
                                                        opts_.pose_prior);
...
template <typename Scalar, int CS>
void Mapper<Scalar,CS>::InitOneFrame(double timestamp, const cv::Mat& img, const cv::Mat& col,
                                     const Features& ft, bool do_mapping)
{
  //Reset();
                                     ...
                                     FrameId kf_id)
{
  ...
  pipeline_controller_.AddFactorTask<work::InitPriorsTask<Scalar,CS>>(fr);
  pipeline_controller_.AddFactorTask<work::PhotometricTask<Scalar,CS>>(kf, fr, opts_.pho_iters,
                                                        cam_pyr_, aligner_);
                                   ...
                                   const std::vector<FrameId>& conns)
{
  ...
  for (auto& id : conns)
  {
    ...
    work::FactorPipelineController::FactorTaskPtr ptr;

    // add photometric error both ways
    if (opts_.use_photometric)
    {
      pipeline_controller_.AddFactorTask<work::PhotometricTask<Scalar,CS>>(kf, back_kf, opts_.pho_iters,
                                                            cam_pyr_, aligner_);
      ptr = pipeline_controller_.AddFactorTask<work::PhotometricTask<Scalar,CS>>(back_kf, kf, opts_.pho_iters,
    ...
    {
      LOG(INFO) << "Adding reprojection loss  kf:" << id;
      pipeline_controller_.AddFactorTask<work::ReprojectionTask<Scalar,CS>>(kf, back_kf, opts_.rep_iters, cam_pyr_[0],
                                                          opts_.rep_max_dist, opts_.rep_huber,
                                                          opts_.rep_sigma, opts_.rep_ransac_maxiters,
                                                          opts_.rep_ransac_threshold);
      ptr = pipeline_controller_.AddFactorTask<work::ReprojectionTask<Scalar,CS>>(back_kf, kf, opts_.rep_iters, cam_pyr_[0],
                                                                opts_.rep_max_dist, opts_.rep_huber,
                                                                opts_.rep_sigma, opts_.rep_ransac_maxiters,
                                                                opts_.rep_ransac_threshold);
    }
    ...
    {
      auto geo_err = std::make_shared<work::GeometricTask<Scalar,CS>>(kf, back_kf, opts_.geo_iters, cam_pyr_[0], opts_.geo_npoints,
                                                                    opts_.geo_huber, opts_.geo_stochastic);

      if (ptr)
        ptr->AddChildTask(geo_err);
      else
        pipeline_controller_.AddFactorTask(geo_err);
    }
...
void Mapper<Scalar,CS>::EnqueueLink(FrameId id0, FrameId id1, Scalar rep_sigma, bool pho, bool rep, bool geo)
{
  ...
  if (pho)
  {
    pipeline_controller_.AddFactorTask<work::PhotometricTask<Scalar,CS>>(kf0, kf1, opts_.pho_iters,
                                                          cam_pyr_, aligner_);
    ptr = pipeline_controller_.AddFactorTask<work::PhotometricTask<Scalar,CS>>(kf1, kf0, opts_.pho_iters,
                                                                cam_pyr_, aligner_, true);
  }
  ...
  if (rep)
  {
    ptr = pipeline_controller_.AddFactorTask<work::ReprojectionTask<Scalar,CS>>(kf0, kf1, opts_.rep_iters, cam_pyr_[0],
                                                        opts_.rep_max_dist, opts_.rep_huber, rep_sigma,
                                                        opts_.rep_ransac_maxiters, opts_.rep_ransac_threshold);
    ptr = pipeline_controller_.AddFactorTask<work::ReprojectionTask<Scalar,CS>>(kf1, kf0, opts_.rep_iters, cam_pyr_[0],
                                                        opts_.rep_max_dist, opts_.rep_huber, rep_sigma,
                                                        opts_.rep_ransac_maxiters, opts_.rep_ransac_threshold);
  }
  ...
  if (geo)
  {
    auto geo_err = std::make_shared<work::GeometricTask<Scalar,CS>>(kf0, kf1, opts_.geo_iters, cam_pyr_[0], opts_.geo_npoints,
                                                                  opts_.geo_huber, opts_.geo_stochastic);
...
//  LOG(INFO) << "Marginalizing past frames";
  ...
  for (const auto& id : frames)
  {
    ...
    if (!fr->marginalized)
    {
      ...
      auto lambda = [&] (work::FactorTask::Ptr w) {
        auto ow = std::dynamic_pointer_cast<work::GenericFactorTask<Scalar>>(w);
        if (!ow) return false;
        VLOG(1) << ow->Name();
      ...
      pipeline_controller_.Purge(lambda);
    }
  }
...
{
  pipeline_controller_.ManagePipeline(new_factors, remove_indices, var_init);
  pipeline_controller_.Update();
  //pipeline_controller_.ManagePipelineWithUpdate(new_factors, remove_indices, var_init);
}
...
//  cv::waitKey(0);
  ...
  LOG(INFO) << " Factors after Book-keeping in MappingStep:" <<new_factors.size();
  //PrintFactors();
...
//  std::cout << std::endl;
  ...
  LOG(INFO) << "Number of frames " << map_->frames.Size() 
           << " KeyFrames " <<  map_->keyframes.Size()
           << " Last KF_ID " << map_->keyframes.LastId() 
           << " has_nonmarginalized_frames:" << has_nonmarginalized_frames;
...
//  }
  ...
  //LOG(INFO) << "Triggering isam graph update";
  ...
  */
  LOG(INFO) << "factorsRecalculated: " << isam_res_.factorsRecalculated;
  LOG(INFO) << "variablesReeliminated: " << isam_res_.variablesReeliminated;
  LOG(INFO) << "variablesRelinearized: " << isam_res_.variablesRelinearized;

  // distribute new factor indices to work items that they originated from
  pipeline_controller_.RehashIndices(isam_res_.newFactorsIndices);
  LOG(INFO) << " Factors after iSAM2 update in MappingStep:" <<new_factors.size();
  ...
  if (isam_res_.variablesRelinearized == 0)
  {
    ...
    pipeline_controller_.SkipRelinearize();
  }
  ...
  //if (VLOG_IS_ON(1))
  if ( VLOG_IS_ON(1))
    pipeline_controller_.PrintEntries();
...
void Mapper<Scalar,CS>::UpdateMap(const gtsam::Values& vals, const gtsam::VectorValues& delta)
{
  ...
  LOG(INFO)<< "Updating Depth/code for changed_kfs count:"<<changed_kfs.size()<<std::endl;
