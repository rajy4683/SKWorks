...
void CameraTracker::TrackFrame(const ImageBufferPyramid& pyr_img1, const GradBufferPyramid& pyr_grad1)
{
  ...
  cv::imshow("tracking residual", residual_);
  cv::imshow("outliers", res_color);
  //std::string res_img= "residual.png"
...
//template <typename Scalar>
//Sophus::SE3f CameraTracker<Scalar>::GetPoseEstimate()
...
//template <typename Scalar>
//void CameraTracker<Scalar>::SetKeyframe(std::shared_ptr<const KeyframeT> kf)
...
//template <typename Scalar>
//void CameraTracker<Scalar>::SetPose(const Sophus::SE3f& pose_wc)
...
//template <typename Scalar>
//void CameraTracker<Scalar>::SetConfig(const TrackerConfig& new_cfg)
...
//template class CameraTracker<float>;
} // namespace df
