...
/*
   Creates the image pyramid and gradients
*/


template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::CreateImgPyrAndGradients(double timestamp, const cv::Mat& frame, bool doupload)
{
  cv::Mat live_col;
  Features features;
...
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::ForceBuildKeyFrames(double timestamp, const cv::Mat& frame )
{
  //const LiveFrameVecT::
  //for (auto liveframe: liveframevector)
  //{
  cv::Mat img_col;
  Features features;
  cv::Mat img_proc = PreprocessImage(frame, img_col, features);
  mapper_->InitOneFrame(timestamp, img_proc, img_col, features, false);
  //}
}
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::PosePerKF()
{
    auto kf_map = mapper_->GetMap();
    std::vector<cv::Mat> array;
    double avg_dpt=2.0f;
 	int start = kf_map->keyframes.Ids().size();
    LOG(INFO) << "No:of KFs:" <<kf_map->keyframes.Ids().size();
    
    for (int i=0;i<start;++i)
    {
    	std::stringstream hw_ss;
        auto kf_entry = kf_map->keyframes.Get(kf_map->keyframes.Ids()[i]);
        LOG(INFO) << "KF ID:" << kf_map->keyframes.Ids()[i] << "Pose:" <<std::endl
                  << kf_entry->pose_wk;
    }
    //cv::imshow("Pyramid level 0", CreateMosaic(array, start*netcfg_.pyramid_levels, 5));
    //while ('q' != cv::waitKey(0));
}

/*
   Displays all the stats of keyframes accumulated
*/

template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::DispKFs()
{
    auto kf_map = mapper_->GetMap();
    std::vector<cv::Mat> array;
    double avg_dpt=2.0f;
    ...
    for (int i=0;i<start;++i)
    {
    	std::stringstream hw_ss;
        auto kf_entry = kf_map->keyframes.Get(kf_map->keyframes.Ids()[i]);
		for(int lvl =0 ; lvl < netcfg_.pyramid_levels; ++lvl)
		{
   		 	int width = kf_entry->pyr_img.GetCpuLevel(lvl).width();
	    	int height = kf_entry->pyr_img.GetCpuLevel(lvl).height();
			cv::Mat img0 = GetOpenCV(kf_entry->pyr_img.GetCpuLevel(lvl));
			cv::Mat dpt0 = GetOpenCV(kf_entry->pyr_dpt.GetCpuLevel(lvl));
			cv::Mat prx0 = apply_colormap(df::DepthToProx(dpt0, avg_dpt));

			cv::Mat std0 = GetOpenCV(kf_entry->pyr_stdev.GetCpuLevel(lvl));
			cv::Mat img_prx_orig = GetOpenCV(kf_entry->pyr_prx_orig.GetCpuLevel(lvl));
			cv::Mat img_vld = GetOpenCV(kf_entry->pyr_vld.GetCpuLevel(lvl));
			cv::Mat jac = GetOpenCV(kf_entry->pyr_jac.GetCpuLevel(lvl));

			array.push_back(img0);
			array.push_back(img_vld);
			array.push_back(prx0);
			array.push_back(std0);
			array.push_back(img_prx_orig);
			//array.push_back(jac);
            hw_ss << "At level:"<< lvl<<std::endl
             << "pyr_img: "<<height<<" "<<width<<std::endl  
             << "pyr_vld: "<<kf_entry->pyr_vld.GetCpuLevel(lvl).height() <<" "<<kf_entry->pyr_vld.GetCpuLevel(lvl).width() <<std::endl  
             << "pyr_dpt: "<<kf_entry->pyr_dpt.GetCpuLevel(lvl).height() <<" "<<kf_entry->pyr_dpt.GetCpuLevel(lvl).width() <<std::endl  
             << "pyr_stdev: "<<kf_entry->pyr_stdev.GetCpuLevel(lvl).height() <<" "<<kf_entry->pyr_stdev.GetCpuLevel(lvl).width() <<std::endl  
             << "pyr_prx_orig: "<<kf_entry->pyr_prx_orig.GetCpuLevel(lvl).height() <<" "<<kf_entry->pyr_prx_orig.GetCpuLevel(lvl).width() <<std::endl  
             << "pyr_jac: "<<kf_entry->pyr_jac.GetCpuLevel(lvl).height() <<" "<<kf_entry->pyr_jac.GetCpuLevel(lvl).width() <<std::endl;
            LOG(INFO) << "Jacobian values:" << kf_entry->pyr_jac.GetCpuLevel(lvl)(1*CS, 1);
		}
        std::cout << hw_ss.str();
        //LOG(INFO) << "KF ID:" << kf_map->keyframes.Ids()[i] << "Pose:" <<std::endl
        //          << kf_entry->pose_wk;
...
/*
   Saves the image pyramid, and gradients
*/
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::SaveCurrentImageAndGrad(std::string dir)
{ 

  //vc::RuntimeBufferPyramidManaged<Scalar, vc::TargetDeviceCUDA> ImagePyrT;
  typedef vc::RuntimeBufferPyramidManaged<Scalar, vc::TargetHost> LocalCpuBufferT;
  typedef Eigen::Matrix<Scalar,1,2> ImageGradT;
  typedef vc::RuntimeBufferPyramidManaged<ImageGradT, vc::TargetHost> LocalGradBuffT;
  //typedef vc::Image2DView<ImageGradT,vc::TargetDeviceCUDA> GradBufferT;

  LocalCpuBufferT cpu_image(netcfg_.pyramid_levels, netcfg_.input_width, netcfg_.input_height);
  LocalGradBuffT grad_image(netcfg_.pyramid_levels, netcfg_.input_width, netcfg_.input_height);
  cpu_image.copyFrom(*pyr_live_img_);
  grad_image.copyFrom(*pyr_live_grad_);
  std::vector<cv::Mat> array;
  std::cout << "Size grad vector 0_0:" <<grad_image(0).width()<< " * " << grad_image(0).height() <<std::endl;
  std::cout << "Size grad vector 0_0:" <<grad_image(1).width()<< " * " << grad_image(1).height() <<std::endl;
  std::cout << "Size grad vector 0_0:" <<grad_image(2).width()<< " * " << grad_image(2).height() <<std::endl;

  cv::Mat grad_mat (grad_image(0).height(), grad_image(0).width(), CV_32FC1, grad_image(0).rawPtr());
  cv::Mat grad_mat1 (grad_image(1).height(), grad_image(1).width(), CV_32FC1, grad_image(1).rawPtr());
  cv::Mat grad_mat2 (grad_image(2).height(), grad_image(2).width(), CV_32FC1, grad_image(2).rawPtr());
  for (int level =0; level< 3;++level)
  {
     ImageGradT grad_image;
     cv::Mat img = GetOpenCV(cpu_image[level]);
     cv::Size s = img.size();
     std::cout<< "Size: "<<s.height << " * "<<s.width<<std::endl;  
     //grad_image.copyFrom(*pyr_live_grad_[level]);
     //cv::Mat grad = GetOpenCV(grad_image(level).matrix());
     array.push_back(img);
     //array.push_back(grad);
  }
  array.push_back(grad_mat);
  array.push_back(grad_mat1);
  array.push_back(grad_mat2);
  //array.push_back(grad_mat);
  cv::imshow("Pyramid level 0", CreateMosaic(array, 2, 3));
                  ...
                  int levels = 1)
{
  ...
  for (int lvl = 0; lvl < levels; ++lvl)
  {
    ...
    for (auto& pair : pairs)
    {
      ...
      cv::Mat img_err = GetOpenCV(kf0->pyr_prx_orig.GetCpuLevel(lvl));
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::Init(df::PinholeCamera<Scalar>& cam, const DeepSlamOptions& opts)
{
  opts_ = opts;
  orig_cam_ = cam;
  ...
  CHECK_EQ(netcfg_.code_size, CS) << "Trying use network with code size different than DeepSlam was compiled for!";
  ...
  if(!opts.use_reprojection)
  {
     LOG(INFO)<<" opts.use_reprojection set to "<<opts.use_reprojection;
     throw;  
  }
...
/* ************************************************************************* */
/*template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::GenerateDebugInfo(std::string dir)
{
  // Generate Debug windows
  // Write all log information
}*/
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::ProcessFrame(double timestamp, const cv::Mat& frame)
{
  if (!bootstrapped_)
  ...
  //SE3T new_pose_wc = tracker_->TrackAndGetNewPose(mapper_, )
  ...
  LOG(INFO) << "New Pose:" << std::endl<< pose_wc_;
...
//  cv::waitKey(2);
  ...
  if (NewFrameRequired())
  {
    ...
    LOG(INFO) << "Adding new frame to keyframe " << curr_kf_;
  ...
  LOG(INFO) << "Inliers:"<<stats_.inliers 
            << " Tracker error:" << stats_.tracker_error 
            << " Distance:" << stats_.distance <<std::endl;
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::BootstrapTwoFrames(double ts1, double ts2, const cv::Mat& img0, const cv::Mat& img1)
{
  CHECK_EQ(img0.type(), CV_8UC3) << "Invalid input image to DeepSlam";
  CHECK_EQ(img1.type(), CV_8UC3) << "Invalid input image to DeepSlam";
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::ForceKeyframe()
{
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::ForceFrame()
{
  force_frame_ = true;
}
...
/* ************************************************************************* */
template <typename Scalar, int CS>
df::PinholeCamera<Scalar> DeepSlam<Scalar,CS>::GetNetworkCam()
{
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::BootstrapOneFrame(double timestamp, const cv::Mat& img)
{
  CHECK_EQ(img.type(), CV_8UC3) << "Invalid input image to DeepSlam";
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::SetOptions(DeepSlamOptions new_opts)
{
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::NotifyPoseObservers()
{
  if (pose_callback_)
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::NotifyMapObservers()
{
  if (map_callback_)
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::NotifyStatsObservers()
{
  if (stats_callback_)
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::SavePostCrashInfo(std::string dir)
{
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::SaveKeyframes(std::string dir)
{
  // create directory
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::SaveResults(std::string dir)
{
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::InitGpu(std::size_t device_id)
{
...
/* ************************************************************************* */
template <typename Scalar, int CS>
void DeepSlam<Scalar,CS>::UploadLiveFrame(const cv::Mat& frame)
{
...
/* ************************************************************************* */
template <typename Scalar, int CS>
cv::Mat DeepSlam<Scalar,CS>::PreprocessImage(const cv::Mat& frame, cv::Mat& out_color, Features& features)
{
...
/* ************************************************************************* */
template <typename Scalar, int CS>
typename DeepSlam<Scalar,CS>::SE3T
DeepSlam<Scalar,CS>::TrackFrame()
{
  SE3T new_pose_wc;
...
/* ************************************************************************* */
template <typename Scalar, int CS>
bool DeepSlam<Scalar,CS>::NewKeyframeRequired()
{
  if (force_keyframe_)
  {
    force_keyframe_ = false;
  ...
  case DeepSlamOptions::AUTO_COMBINED:
  {
    bool inlier_bad = inliers < opts_.inlier_threshold;
...
/* ************************************************************************* */
template <typename Scalar, int CS>
bool DeepSlam<Scalar,CS>::NewFrameRequired()
{
  if (force_frame_)
  {
    force_frame_ = false;
  ...
  if (opts_.keyframe_mode == DeepSlamOptions::NEVER)
    return false;
...
/* ************************************************************************* */
template <typename Scalar, int CS>
typename DeepSlam<Scalar,CS>::KeyframeId DeepSlam<Scalar,CS>::SelectKeyframe()
{
  if (mapper_->NumKeyframes() == 0)
    LOG(FATAL) << "KeyframeMap is empty (should not happen here)";

  KeyframeId kfid = 0;
  if (opts_.tracking_mode == DeepSlamOptions::LAST)
  {
    kfid = mapper_->GetMap()->keyframes.LastId();
  }
  else if (opts_.tracking_mode == DeepSlamOptions::CLOSEST)
  {
  ...
  else if (opts_.tracking_mode == DeepSlamOptions::FIRST)
  {
...
/* ************************************************************************* */
template <typename Scalar, int CS>
bool DeepSlam<Scalar,CS>::CheckTrackingLost(const SE3T& pose_wc)
{
...
template class DeepSlam<float,DF_CODE_SIZE>;
