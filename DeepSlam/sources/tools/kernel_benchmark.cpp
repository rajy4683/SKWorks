               ...
               std::shared_ptr<df::DecoderNetwork> decoder_net)
{
  ...
  LOG(INFO) << "Create Pyramid started for:" << img_path;
  for (int i = 0; i < pyrlevels; ++i)
  {
    ...
    if (i == 0)
    {
      ...
      LOG(INFO) << "Index 0 Sobel gradients will be attempted with:" << img_path;
...
int main(int argc, char** argv)
{
  ...
  LOG(INFO) << "Decoder object created";

  // create sfmaligner
  auto aligner = std::make_shared<df::SfmAligner<float,CS>>();
  LOG(INFO) << "Sfm Aligner loaded to CUDA" ;
  ...
  LOG(INFO) << "Image Sequence received";
